...
const termsOfService = LegalDocument(
  ...
  sections: [
    ...
    LegalSection(
      '2. 익명성',
      '가입에는 이메일이나 전화번호가 필요하지 않습니다. 내 정보 > 계정 보호 설정에서 아이디와 '
          '비밀번호를 정해 두지 않으면, 앱을 삭제하거나 기기를 바꿀 때 계정을 복구할 수 없습니다. '
          '아이디·비밀번호를 정해 두었다면 그 정보로 다시 로그인할 수 있으며, 이 경우에도 이메일이나 '
          '전화번호는 여전히 수집하지 않습니다. 이용자는 언제든지 내 정보 화면에서 계정과 작성한 내용을 '
          '삭제할 수 있습니다.',
    ),
    LegalSection(
      '3. 금지 행위와 무관용 원칙',
  ...
  sections: [
    LegalSection(
      ...
      '• 익명 계정 식별자(자동 생성)\n'
          ...
          '• (선택) 계정 보호를 위해 이용자가 정한 아이디와 비밀번호. 비밀번호는 원문이 저장되지 않고 '
          '암호화되어 저장되며, 운영자를 포함해 누구도 원래 비밀번호를 확인할 수 없습니다.\n'
          '이메일, 전화번호, 실명은 수집하지 않으며, 아이디도 실제 이메일이나 전화번호일 필요가 없습니다.',
    ),
    LegalSection(
      '2. 이용 목적',
